...
static inline u32 checkDecryptionWrapper(u8* addr, u32 match_ret, u32 mismatch_ret) {
	...
	if (addr[offset++] != 0xF0) return mismatch_ret;
	if (addr[offset++] != 0x00) return mismatch_ret;
	if (addr[offset++] != 0x2D) return mismatch_ret;
	if (addr[offset++] != 0xE9) return mismatch_ret;
	...
	if (addr[offset++] != 0xF0) return mismatch_ret;
	if (addr[offset++] != 0x00) return mismatch_ret;
	if (addr[offset++] != 0xBD) return mismatch_ret;
	if (addr[offset++] != 0xE8) return mismatch_ret;
	
	if (addr[offset++] != 0x60) return mismatch_ret;
	if (addr[offset++] != 0x10) return mismatch_ret;
	if (addr[offset++] != 0x9F) return mismatch_ret;
	if (addr[offset++] != 0xE5) return mismatch_ret;
