...
Before encoding begins, `xorval` is initialized to a value of `0xF0B9A2EA`.
...
- The three least significant bytes are incremented by `0x402`.

- A relocation for this instruction has its addend incremented by `0x1000` (and subtraction by 8 removed).
...
- The three least significant bytes are decremented by `0x402`.
...
- The three least significant bytes are split apart:
	...
	- `RC4.x` is updated to be the encoded byte from the previous step.
	- The third-least-significant byte is substituted via an [S-box](https://en.wikipedia.org/wiki/S-box). However, this S-box is very weak: `S[i] = i ⊕ 0xFF`. It is unknown why this was implemented using an S-box instead of simply exclusive-oring the value `0xFF` directly.
...
Note the association between the two values `0x1000` and `0x402`. When encoding, the addition of `0x1000` may be applied to the instruction directly, or to the relocation for that instruction. When the relocation is encoded this way and then applied to the instruction, the offset of `0x1000` bytes is converted into a offset of *instructions*, effectively dividing it by 4. ARM branches, like the ARM PC, are also always offset by 2: a branch of 0 (`EB000000`) is a jump forward by 8 bytes or 2 instructions (`bl #8`). So, when an addition of `0x1000` is applied from the relocation into the instruction, it is divided by 4 and then incremented by 2, resulting in an addition of `0x402` to the instruction.

Therefore, if the encoding procedure added `0x1000` to the *relocation*, the decoding procedure needs to subtract `0x402` from the instruction.

Meanwhile, if the encoding procedure added `0x1000` to the *instruction*, the decoding procedure needs to (obviously) subtract `0x1000` from the instruction.

*(In my opinion the smarter way to do this would have been to encode the relocation by incrementing it by `(0x1000 - 2) * 4 = 0x3FF8`, such that the resulting instruction would be incremented by `0x1000` and could then be decoded normally)*
...
```
EA000402    b   4112    R_ARM_PC24:  bar+0x1000
E1965D1F    orrs r5, r6, pc, lsl sp
```

Where, as discussed in the previous section, incrementing the relocation by `0x1000` will increment the instruction by `0x402`.

Therefore, after the reference is linked and the relocation is applied, the branch destination can still be correctly decoded by subtracting `0x402`.
...
Where `obfuscated_function_address` is the function address of the target function that is to be encrypted, obfuscated by incrementation by `0x1000`, as described in the above reference obfuscation description.
...
This newly modified seed key is expanded into a 128-bit RC4 key as normal. It is also saved as the seed key (after obfuscating it via incrementing by `&BSS + 0x1000`) to be used for the next decryption of the function.

For example, if the library is conducting a ROM read test, it will perform 6 calls to the decryption wrapper function `RunEncrypted_ROMUtil_CRC32()`, which will change the key 6 times in total. If the original seed key is `0xC20D`, and the address and size of the inner `RunEncrypted_ROMUtil_CRC32()` is `0x02300000` + `0x14C`, then the subsequent seed keys and expanded keys will be:
