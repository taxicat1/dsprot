...
Before running any of the core check functions, it also conducts an integrity check. It checks the first 13-16 bytes (within 4 instructions) of the code of the function against its own hardcoded record, and if it mismatches, it will automatically report that a flashcart or emulator was detected. There is no integrity check on the dummy detection functions.
