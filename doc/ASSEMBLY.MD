...
Decryption wrappers, decoding static initializers, a helper function to clear the data and instruction cache, and the crashing function are all written at least partially in assembly. This can be proven. All other functions in the library may or may not be written partially or wholly in assembly, but they all may be decompiled to some C code which matches, and therefore can be assumed to have originally been written in C.
...
NitroStaticInit:
	...
	.word RC4_InitAndEncryptInstructions,  0x74
	.word RC4_InitAndDecryptInstructions,  0x74
	.word RC4_Byte,                        0x58
	.word RC4_CategorizeInstruction,       0x3C
...
The crash function takes the form:

```asm
	arm_func_start Crash
Crash:
	mov    r10, pc
	orrs   r10, r10, r10
	eorne  lr, lr, lr
	subs   r10, r10, #8
	ldr    r9, =MI_CpuClear32+0x3200
	mov    r0, sp
	sub    r9, r9, #0x3200
	mov    r1, #0x100
	blx    r9
	ldr    lr, =OS_Terminate+0x3200
	mov    r0, r10
	mov    r1, #0x1000
	sub    lr, lr, #0x3200
	bx     r9
...
This has multiple obvious giveaways that it is assembly:
- It does not preserve any registers on the stack, clobbering `r9`, `r10`, and `lr` as it uses them.
- It moves `pc` into another register for manipulation.
- It has no proper return, instead jumping away into another function after overwriting `lr` (though this is very intentional).
