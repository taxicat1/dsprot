...
Encryptor_DecryptionWrapperFragment:
	...
	str    r10, [ip, #0x10]
	mov    r10, ip
	ldr    ip, =Proxy_Encryptor_DecryptFunction  ; Load and deobfuscate address of decryption routine
	str    lr, [r10]
	ldr    ip, [ip]
	ldmib  r10, {r0-r2}
	sub    ip, ip, #0x3200
	blx    ip                                    ; Run function decryption routine
	mov    ip, r0
	ldmia  sp!, {r0-r3}
	blx    ip                                    ; Run internal function
	ldr    ip, =Proxy_Encryptor_EncryptFunction  ; Load and deobfuscate function re-encryption routine
	stmdb  sp!, {r4}
	ldr    ip, [ip]
	mov    r4, r0
	sub    ip, ip, #0x3200
	ldmib  r10, {r0-r2}
	blx    ip                                    ; Run function re-encryption routine
	str    r0, [r10, #0x4]
	mov    r0, r4
	ldmia  sp!, {r4}
	ldr    lr, [r10]
...
```asm
	arm_func_start RunEncrypted_ROMTest_IsBad
RunEncrypted_ROMTest_IsBad:
	...
	mov      ip, ip, lsr #6
	mov      r0, ip
hash_loop:                              ; Hash `Encryptor_DecryptionWrapperFragment`
	ldr      r2, [r0]
	mov      r4, r2, lsr #24
	cmp      r4, #0xEA
	cmpne    r4, #0xEB
	eorne    r3, r3, r2, ror #17
	addne    r3, r3, r2, ror #28
	eorne    r3, r3, r2, ror r1
	subs     r1, r1, #1
	add      r0, r0, #4
	bne      hash_loop
	streq    r3, [pc, #0x14]           ; Store resulting hash in data structure
	ldmeqfd  sp!, {r0, r1, r2, r3, r4}
	ldmnefd  sp!, {r5, r6, r7, r8, sb}
	stmeqdb  sp!, {ip}
	orr      ip, pc, pc                ; Set `ip` to point at data structure
	ldmeqia  sp!, {pc}                 ; Call `Encryptor_DecryptionWrapperFragment`
	.word BSS + 1                      ; Dummy storage space
	.word BSS + 3                      ; Dummy storage space 
	.word ROMtest_IsBad                ; Function to decrypt-run-encrypt
	.word 0x540                        ; Function size
	.word 0                            ; Dummy storage space
	.word Encryptor_DecryptionWrapperFragment + 0x68000000
	arm_func_end RunEncrypted_ROMTest_IsBad
```
...
- The "body" function also pre-supposes an appropriate data structure containing metadata for the inner function (decryption key, size, and address) has been loaded into `ip`. C would not assume this, it would require the address be passed into the function using `r0-r3` or the stack.
...
```asm
	arm_func_start RC4_DecodeFunctions
RC4_DecodeFunctions:
	orr   r0, pc, #0x0
	adds  r0, r0, #0x4
	...
	arm_func_end RC4_DecodeFunctions
```
...
There is a separate form of decoding initializer, here called the primary initializer, which will call other initializers, erase its own instructions from RAM os it cannot be run again, and then finally perform its own decoding. This must be called after loading DS Protect, taking the place of `NitroStaticInit` functions that were run automatially.

The primary initializer takes the form:

```asm
	arm_func_start DSProt_DecodeFunctions
DSProt_DecodeFunctions:
	stmdb sp!, {lr}
	bl CoreTests_DecodeFunctions            ; Call all other decoders
	bl Encryptor_DecodeFunctions
	bl Integrity_DecodeFunctions
	bl RC4_DecodeFunctions
	ldmfd sp!, {lr}
	ldr r0, =DSProt_DecodeFunctions+0x3200
	ldr r1, =0xE12FFF1E                     ; <bx lr> instruction to write
	sub r0, r0, #0x3200
	stmia r0!, {r1-r10}                     ; Clobber the first 10 instructions of itself
	orr   r0, pc, #0x0                      ; Continue with normal decoding as above
	adds  r0, r0, #0x4
	bne   Encryptor_DecodeFunctionTable
	.word DSProt_DetectFlashcartA, 0x24
	.word DSProt_DetectEmulatorA, 0x24
	.word DSProt_DetectFlashcartB, 0x24
	.word DSProt_DetectEmulatorB, 0x24
	.word 0, 0
	.word Garbage
	arm_func_end DSProt_DecodeFunctions
```

This has an additional giveaway that it is assembly: rather than using any kind of fixed data or looping, it uses whatever data happens to be in registers `r1-r10` to clobber itself.
